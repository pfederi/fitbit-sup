...
function createPreflightCheck() {
	...
	return function (ctx, options, file) {
		if (preflightCheckResults[file] === undefined) {
			...
			options.filename = path.join(path.dirname(file), 'x.js');

			var inputCode = 'class Foo extends Bar {};\nexport default Foo;';
			var transformed = core.transform(inputCode, options);
...
function babel(options) {
	...
	return {
		...
		load: function load(id) {
			...
			return core.buildExternalHelpers(externalHelpersWhitelist, 'module');
		},
		...
		transform: function transform$1(code, id) {
			...
			var helpers = preflightCheck(this, babelOptions, id);
			...
			var localOpts = Object.assign({}, {filename: id},
				babelOptions,
				{plugins: helpers !== RUNTIME ? babelOptions.plugins.concat( [importHelperPlugin]) : babelOptions.plugins});
